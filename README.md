...
**MAC Address Spoofing** isn't usually needed in Qubes because only your net-vm "touches" physical hardware like your router or modem. The reason I have Firewall VM coded to spoof it on start, is because I use Qubes from networks other than my home wifi occassionally. Hotels, Office, School... Even though your AppVM's have their MAC addresses abstracted a as result of compartmentalization, and their networking being routed from net-vm downstream... it's still feasible that your net-vm could get tracked. (That's partially why I have the next VM where it is in the chains order. To serve as a "bottom up buffer" against Kali users & wifi networks I'm not in control of). 💀
